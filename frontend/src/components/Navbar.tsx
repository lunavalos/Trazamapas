...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-[1280px] mx-auto flex items-center gap-3">
        ...
        <div className="flex-1 bg-surface-white border border-mist-border/70 rounded-[16px] shadow-sm h-[64px] px-5 relative flex items-center justify-between">
          ...
          <motion.a 
            ...
            className="hidden lg:inline-block px-5 py-2 rounded-[10px] bg-brand-primary text-white font-semibold hover:bg-[#3d0075] transition-all duration-150 text-sm shadow-sm"
          >
            Cotizar Viaje
          </motion.a>
          ...
          <button 
            ...
            className="lg:hidden w-9 h-9 rounded-[10px] border border-mist-border text-cinder flex items-center justify-center hover:bg-surface-lift transition cursor-pointer"
      ...
      {/* Mobile/Tablet Drawer Dropdown (Absolute overlay: floats over content without pushing containers down) */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden absolute top-[calc(100%+8px)] left-5 sm:left-6 right-5 sm:right-6 max-w-[1280px] mx-auto bg-surface-white rounded-[16px] border border-mist-border/70 p-5 md:py-3.5 md:px-5 shadow-2xl z-50 flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4"
          >
            {/* Nav links: Vertical on mobile (<768px), Horizontal and left-aligned on tablet (768px-1024px) */}
            <nav className="flex flex-col md:flex-row md:items-center gap-1 md:gap-1.5 font-sans">
              <Link 
                href="/" 
                onClick={() => setMobileMenuOpen(false)} 
                className="text-cinder font-semibold text-sm py-2 md:py-1.5 md:px-3 rounded-[10px] border-b border-mist-border/30 md:border-b-0 hover:bg-brand-primary hover:text-white transition-all duration-150"
              >
                Inicio
              </Link>
              <Link 
            ...
            {/* Actions: On mobile (<768px) Cotizar Viaje + Social Bubbles; On tablet (768px-1024px) Cotizar Viaje aligned right and social bubbles hidden */}
            <div className="flex items-center justify-between pt-2 md:pt-0 shrink-0">
              <motion.a 
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                href="https://wa.me/528443409914?text=Hola,%20quisiera%20cotizar%20un%20viaje" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-brand-primary text-white text-center font-semibold text-sm py-2.5 md:py-2 px-4 md:px-6 rounded-[10px] flex-1 md:flex-initial md:w-auto hover:bg-[#3d0075] transition-all duration-150 shadow-sm mr-3 md:mr-0"
              >
                Cotizar Viaje
              </motion.a>

              {/* Social bubbles: Visible ONLY on mobile (<768px), strictly hidden on tablet and desktop */}
              <div className="flex items-center gap-2 md:!hidden">
                <a 
                  href="https://wa.me/528443409914" 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="w-9 h-9 rounded-full bg-brand-primary text-white flex items-center justify-center hover:bg-brand-primary/90 transition-all shadow-sm"
                  aria-label="WhatsApp TrazaMapas"
                  title="WhatsApp"
                >
